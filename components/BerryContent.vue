<template>
  <v-container class="tw-py-16">
    <v-row justify="start" align="center" class="tw-mb-10">
      <v-col cols="12" sm="8" md="6">
        ...
        <v-text-field
          ...
          :disabled="loading"
          hide-details="auto"
          label="Cari nama berry disini"
          append-outer-icon="mdi-refresh"
          @keypress.enter="searchData()"
          @click:append-outer="getAllBerries(), clearInput()"
        ></v-text-field>
      </v-col>
    </v-row>
    <v-row>
      <v-col
        v-for="(data, i) in $store.state.berry.berries"
        :key="i"
        cols="12"
        sm="6"
        md="3"
          ...
          <v-row>
            <v-col cols="12" sm="12" md="12">
              ...
              <v-btn
                ...
                block
                color="#fff"
                :to="{ name: 'berry-slug', params: { slug: data.name } }"
    ...
    <div class="text-center tw-mt-4">
      <v-btn
        v-if="
          $store.state.berry.next !== null &&
          !loading &&
          !$store.state.berry.search
        "
        depressed
        large
        dark
      ...
      <v-btn v-if="loading && !$store.state.berry.search" text large dark>
...
export default {
  ...
  methods: {
    ...
    async getAllBerries() {
      await this.$store.dispatch('berry/getBerries')
    },
    async loadMore() {
      this.loading = true
      const urlNext = this.$store.state.berry.next.split('?')[1]
      await this.$store.dispatch('berry/addBerryMore', urlNext)
      this.page++
      this.loading = false
    },
    async searchData() {
      this.loading = true
      await this.$store.dispatch('berry/searchBerry', this.search)
      this.loading = false
    },
  },
}
